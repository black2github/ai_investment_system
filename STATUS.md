...
## 25.09 поздно: РЕШЕНИЕ ПО ВЕРОЯТНОСТЯМ СЦЕНАРИЕВ ЗАПИСАНО — точка возврата перед сменой модели

Решение владельца по DR-2026-09-25-01 ЗАПИСАНО: вариант V1, p(TAIWAN_SEIZURE) = 0.10, p(CHIP_COLD_WAR) = 0.07, BASE
0.83 = «гонка продолжается без паритета и без силового Тайваня» (холодная война как фон ~90 % — базовый режим, не
сценарий). Записи: _portfolio.yaml → owner_decisions (DR-2026-09-25-01/p_TS, /p_CW, /BASE),
portfolio/_scenarios/*_v1.0.yaml → probability + owner_judgment (rationale в кавычках — двоеточие ломало YAML), DR
(раздел «Решение владельца»), decisions/DR-2026-09-25-01-owner-analysis.md (разбор владельца дословно). Скрипт:
scratchpad _apply_decision_dr250925.py. ПРОВЕРКА ВАЛИДАТОРОМ (контракт вероятностей SCN-010) ещё не прогнана после
записи. ТОЧКА ВОЗВРАТА (лимит модели): всё принятое закоммичено в edf9ca2 (workspace) и 15d1dda (lab); НЕ закоммичено
— решение владельца (4 файла выше) и этот STATUS. ОЧЕРЕДЬ: (1) валидатор scenario с вероятностями → ожидается pass,
probability_status owner_judgment; (2) смесь нормативно: portfolio_paths с probabilities (51764c/60045a перепрогнать с
save) — ориентир при 0.10/0.07: текущие +21.8 % / ES5 +6 %, оптимум 4 +15.2 % / +15 %; (3) СРЕЗ ДВИЖКА «смесь путей»:
portfolio_paths режим mixture_export — разбиение path_id по сценариям (BASE [0,k_B), TS [k_B,k_B+k_T), CW остаток; k
чётные, одни диапазоны для всех компаний → совместная структура сохраняется; meta с global_seed/chunk/paths/joint как
у исходных) → файлы …-mixture-<TK>-paths.npz, на них без изменений работают оптимизатор и Stability; тест: метрики
файла-смеси ≈ взвешенная смесь §6; (4) оптимизатор на смеси с 15 бумагами (_opt_inputs.json дополнить CRWV/ASTS/SPOT,
снять их из fixed_weights); (5) §3.3 Stability как скрипт: смеси при ×0.75 / ×1.25 / +10 п.п. на сценарий →
оптимизатор ×5, сравнить веса — долг: перенести в движок; (6) IMMA: приёмка joint-v11-reissues.feedback.md дополнить
3.5 (содержит ли BASE гонку: госзаказ, reshoring, ставки) и 3.6 (составной сценарий: прорыв Китая снимает китайский
сдерживающий фактор), заказ TAIWAN_QUARANTINE (серая зона: ограничения → карантин/досмотры → нормализация, без
конфликта; 15–20 % по владельцу; те же драйверы мягче) — файл ещё не написан; потом заказ Lifecycle; (7) дозор:
признаки владельца (суда береговой охраны, ro-ro, продажа оружия $14 млрд, Patriot/LRASM, китайская EUV, тиражи
литографов) добавить в каталог фаз. Открытые вопросы к IMMA: 3.1 migration delta NBIS, 3.2 доля bridge-базы, 3.4
ScenarioConcentration = 100 %, предрасчёт RV SPOT (чистый кэш).

## 25.09 ночь: ЗАХОД 5 — ОПТИМИЗАТОР НА СМЕСИ (15 бумаг), срез mixture_export, заказ TAIWAN_QUARANTINE

Решение владельца закоммичено (workspace 8f8b9be). СРЕЗ ДВИЖКА: portfolio_paths 1.3.0 — режим mixture_export:
разбиение path_id по сценариям (BASE [0,k_B), далее сценарии; k чётные; одни диапазоны у всех компаний; meta BASE +
mixture) → файлы …-mixture-<TK>-paths.npz, на них без изменений работают оптимизатор и Stability; тест
test_mixture_export_partition (метрики файла ≈ взвешенная смесь §6): test_portfolio_paths 5/5; полный набор lab 118
passed + 2 flaky под нагрузкой (stability, validator scenario — изолированно проходят; тест scenario-режима починен:
брал nvda v1.0.2, теперь текущую версию). ЗАХОД 5 (_portfolio_run5.py, 15 бумаг, смесь 0.10/0.07): экспорт …-5ab3b3
(BASE 415k / TS 50k / CW 35k путей); сверка: смесь на файлах …-0f4b08 median5 +21.76 % / P(l30) 0.0024 / ES5 +6.0 %
против взвешенной §6 …-c78b12 +21.76 % / 0.0026 / +6.0 % — совпало. ОПТИМИЗАТОР …-92a19f (стадия A, режим Stress,
потолки Conviction Overlay по BASE-нормативам, fixed только GLD/UFO): feasible; веса NBIS 21.5 %, NVDA 14 % (cap),
SPOT 13.5 % (cap), LLY 10.5 %, RKLB 8 % (cap), CRWV 5.5 %, NET 2 %, PLTR 2 %, SPCX 1 %, кэш 19.6 %;
META/MSFT/ETN/ASML/HOOD/ASTS = 0; медиана CAGR 5Y 19.6 %, ES5 +30 %, turnover 45 %; связывающие: common_cause
TAIWAN_SUPPLY_DISRUPTION, потолки NVDA/RKLB/SPOT. Против захода 4 (BASE, 12 бумаг: ETN 7.5, META 10, MSFT 4 → 0; SPOT
0 → 13.5, CRWV 0 → 5.5): оптимизатор тянется к именам с узким нижним хвостом (SPOT W 0.232 ниже ориентира, CRWV,
NBIS/NVDA после parity-gated) — прямое следствие открытых вопросов 3.1/3.2 и W-предупреждений; результат считать
УСЛОВНЫМ до ответа IMMA, не основанием для сделок. §3.3 (×0.75/×1.25/+10 п.п.) — запущен скриптом (--sens), результаты
ниже отдельной записью. IMMA: заказ to_imma/scenario-taiwan-quarantine.request.md (TAIWAN_QUARANTINE v1.0: фазы
RESTRICTIONS → QUARANTINE → NORMALIZATION_OR_FROZEN, каталог признаков владельца; вопрос 3.5 — содержит ли BASE гонку;
вопрос 3.6 — составной сценарий / условные вероятности); очередь отправки: приёмка joint-v11-reissues.feedback.md →
заказ QUARANTINE → заказ Lifecycle.

## 25.09 ночь: §3.3 — ОПТИМУМ НЕЧУВСТВИТЕЛЕН К СДВИГАМ ВЕРОЯТНОСТЕЙ СЦЕНАРИЕВ

§3.3 по вероятностям (скриптом --sens, 4 смеси → оптимизатор): ×0.75 (0.075/0.0525) …-eb6728, ×1.25 (0.125/0.0875)
…-369e34, one-scenario-up TS +10 п.п. (0.20/0.07) …-18a304, CW +10 п.п. (0.10/0.17) …-146364 — ВЕСА ОПТИМУМА НЕ
МЕНЯЮТСЯ ни в одном варианте (Δ = 0 по всем бумагам и кэшу): решение зажато ограничениями (common_cause
TAIWAN_SUPPLY_DISRUPTION 35 %, потолки NVDA/RKLB/SPOT), а вклад сценариев при таких вероятностях мал; меняются только
метрики: медиана 19.0–19.7 %, ES5 +22 % (TS 0.20) … +32 % (×0.75). Вывод для §3.3: портфель устойчив к сдвигам
